...
func (r *Request) RealIP() string {
	ra := r.RemoteAddress()
	if ip := r.Header().Get(vodka.HeaderXForwardedFor); ip != "" {
		ra = ip
	} else if ip := r.Header().Get(vodka.HeaderXRealIP); ip != "" {
		ra = ip
	} else {
...
func (r *Request) FormParams() map[string][]string {
	if strings.HasPrefix(r.header.Get(vodka.HeaderContentType), vodka.MIMEMultipartForm) {
		if err := r.ParseMultipartForm(defaultMemory); err != nil {
			panic(fmt.Sprintf("vodka: %v", err))
		}
	} else {
...
func (r *Request) Cookie(name string) (engine.Cookie, error) {
	...
	if err != nil {
		return nil, vodka.ErrCookieNotFound
	}
	return &Cookie{c}, nil
}
