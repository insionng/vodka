...
const MultipartRequest = `POST /insionng/vodka HTTP/1.1
Host: github.com
Connection: close
...
func RequestTest(t *testing.T, request engine.Request) {
	...
	request.SetHost("insionng.com")
	assert.Equal(t, "insionng.com", request.Host())
	request.SetURI("/insionng/vodka?token=54321")
	assert.Equal(t, "/insionng/vodka?token=54321", request.URI())
	assert.Equal(t, "/insionng/vodka", request.URL().Path())
